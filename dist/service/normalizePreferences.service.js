import { movieGenres } from "../constants/movie.types.js";
 ...
 */
export async function normalizePreferences(preferences) {
    const genreIds = preferences.genres?.map((genre) => movieGenres[genre].toString()) ?? [];
    const excludeGenreIds = preferences.excludeGenres?.map((genre) => movieGenres[genre].toString()) ?? [];
    const actorIds = await Promise.all((preferences.cast ?? []).map(async (cast) => {
        const result = await searchPersonByName(cast);
        return result?.id;
    }));
    const crewIds = await Promise.all((preferences.crew ?? []).map(async (crew) => {
